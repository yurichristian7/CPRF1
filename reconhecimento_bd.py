    ...
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS registros_reconhecimento (
                id INT AUTO_INCREMENT PRIMARY KEY,
                pessoa_id INT,
                nome_reconhecido VARCHAR(100) NOT NULL,
                confianca DECIMAL(5,2) NOT NULL,
                data_hora TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                coordenada_x INT,
                coordenada_y INT,
                arquivo_rosto VARCHAR(255),
...
def save_recognition_to_db(nome, confianca, coordenada_x, coordenada_y, arquivo_rosto=None):
    ...
    try:
        ...
        # Se não for "Desconhecido", tenta obter o ID da pessoa
        pessoa_id = None
        if nome.lower() != "desconhecido":
            pessoa_id = get_person_id(nome)
            # Se a pessoa não existe no banco, insere
            if pessoa_id is None:
                pessoa_id = insert_person(nome)
        
        # Insere o registro de reconhecimento
        cursor.execute("""
            INSERT INTO registros_reconhecimento 
            (pessoa_id, nome_reconhecido, confianca, coordenada_x, coordenada_y, arquivo_rosto) 
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (pessoa_id, nome, confianca, coordenada_x, coordenada_y, arquivo_rosto))
