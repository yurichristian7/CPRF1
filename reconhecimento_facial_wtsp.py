...
WHATSAPP_CONFIG = {
    "phone_number": "+5531986091013",  # Número do responsável no formato internacional
...
def enviar_mensagem_whatsapp_instantanea(nome_pessoa, confianca):
    """
    Versão alternativa que envia mensagem instantânea
    """
    try:
        mensagem = f"🔍 *Sistema de Reconhecimento Facial*\n\n👤 *Pessoa reconhecida:* {nome_pessoa}\n📊 *Confiança:* {int(confianca)}%\n⏰ *Horário:* {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}\n\nSistema funcionando normalmente!"
...
def enviar_alerta_desconhecido():
    """
    Envia alerta quando pessoa desconhecida é detectada
    """
    try:
        ...
        print(f"📱 Agendando ALERTA WhatsApp para {hora}:{minuto:02d}")
...
def enviar_alerta_desconhecido_instantaneo():
    """
    Versão alternativa que envia alerta instantâneo
    """
    try:
        mensagem = f"🚨 *ALERTA - Pessoa Desconhecida*\n\n⚠️ Foi detectada uma pessoa não cadastrada no sistema!\n⏰ *Horário:* {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}\n📍 *Local:* Câmera de Reconhecimento\n\n🔍 *Ação necessária:* Verificar imediatamente!"
